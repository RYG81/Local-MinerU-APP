...
async def maybe_repair_quiz_output(
    file_path,
    file_suffix,
    local_md_dir,
    file_name,
    ...
    """Generate validated native-text quiz output for born-digital PDFs."""
    if not enabled or file_suffix != "pdf":
        return None, None
    ...
    try:
        ...
        repair_script = project_root / "scripts" / "repair_quiz_extraction.py"
        spec = importlib.util.spec_from_file_location(
            "mineru_local_quiz_repair", repair_script
        )
        if spec is None or spec.loader is None:
            raise ImportError(f"Cannot load quiz repair module: {repair_script}")
        repair_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(repair_module)
        repair_quiz_pdf = repair_module.repair_quiz_pdf
    except Exception as exc:
        logger.warning(f"Quiz repair is unavailable: {exc}")
        return None, None

    if status_callback is not None:
        status_callback("Repairing quiz extraction...")

    output_prefix = Path(local_md_dir) / f"{file_name}_quiz_repaired"
    mineru_content_v2 = Path(local_md_dir) / f"{file_name}_content_list_v2.json"
    try:
        report = await asyncio.to_thread(
            ...
            mineru_content_v2=mineru_content_v2 if mineru_content_v2.is_file() else None,
            max_pages=max_pages,
        )
    except Exception as exc:
        logger.warning(f"Quiz repair failed for {file_name}: {exc}")
        return None, None

    if not report.get("valid"):
        logger.warning(f"Quiz repair validation failed for {file_name}: {report}")
        return None, None

    repaired_md = output_prefix.with_suffix(".md")
    repaired_content = output_prefix.with_name(
        f"{output_prefix.name}_content_list.json"
    )
    repaired_content_v2 = output_prefix.with_name(
        f"{output_prefix.name}_content_list_v2.json"
    )
    promoted_files = [
        (repaired_md, Path(local_md_dir) / f"{file_name}.md"),
        (repaired_content, Path(local_md_dir) / f"{file_name}_content_list.json"),
        (
            repaired_content_v2,
            Path(local_md_dir) / f"{file_name}_content_list_v2.json",
        ),
    ]
    for repaired_path, canonical_path in promoted_files:
        if not repaired_path.is_file():
            logger.warning(f"Missing repaired quiz artifact: {repaired_path}")
            return None, None
        backup_path = canonical_path.with_name(
            f"{canonical_path.stem}_mineru_original{canonical_path.suffix}"
        )
        if canonical_path.is_file() and not backup_path.exists():
            shutil.copy2(canonical_path, backup_path)
        shutil.copy2(repaired_path, canonical_path)

    logger.info(
        f"Quiz repair completed for {file_name}: "
        f"{report.get('extracted_questions')} questions, "
        f"strategy={report.get('selected_strategy')}"
    )
    return (
        Path(local_md_dir) / f"{file_name}.md",
        Path(local_md_dir) / f"{file_name}_content_list_v2.json",
    )
...
async def _run_to_markdown_job(
    ...
    content_list_path = Path(local_md_dir) / f"{file_name}_content_list.json"
    repaired_md_path, repaired_content_path = await maybe_repair_quiz_output(
        file_path=file_path,
        file_suffix=file_suffix,
        local_md_dir=local_md_dir,
        file_name=file_name,
        enabled=quiz_repair,
        expected_options=quiz_options,
        max_pages=end_pages,
        status_callback=emit_status,
    )
    if repaired_md_path is not None:
        md_path = repaired_md_path
    if repaired_content_path is not None:
        content_list_path = repaired_content_path
    ...
    if content_list_path.is_file():
        content_list_json = content_list_path.read_text(encoding="utf-8")
    else:
...
def main(ctx,
    ...
    i18n = gr.I18n(
        en={
            ...
            "quiz_repair": "Repair quiz extraction",
            "quiz_repair_info": "For born-digital quiz PDFs, validate question order/options and repair complex multi-column pages using the native text layer.",
            "quiz_options": "Options per question (Auto recommended)",
            "convert": "Convert",
            "clear": "Clear",
        ...
        zh={
            ...
            "quiz_repair": "修复试题提取",
            "quiz_repair_info": "针对原生 PDF 试题，校验题号和选项，并使用文本坐标修复复杂多栏页面。",
            "quiz_options": "每题选项数（推荐自动）",
            "convert": "转换",
            "clear": "清除",
            "doc_preview": "文档预览",
            "examples": "示例：",
    ...
    with gr.Blocks(**_blocks_kwargs) as demo:
        ...
        with gr.Column(elem_classes=["mineru-advanced-popover"]):
            with gr.Column(elem_classes=["mineru-advanced-card"]):
                ...
                with gr.Group():
                    quiz_repair = gr.Checkbox(
                        label=i18n("quiz_repair"),
                        value=False,
                        info=i18n("quiz_repair_info"),
                    )
                    quiz_options = gr.Dropdown(
                        choices=["auto", "2", "3", "4", "5", "6", "7", "8"],
                        value="auto",
                        label=i18n("quiz_options"),
                    )
        ...
        clear_bu.add([input_file, md, doc_show, md_text, content_list_json, output_file, is_ocr, quiz_repair, office_html, status_panel])
        ...
        change_bu.click(
            ...
            inputs=[input_file, max_pages, is_ocr, formula_enable, table_enable, image_analysis, hybrid_effort, language, backend, url, quiz_repair, quiz_options],
